...
import {
  ...
  targetDisplay,
  windowBounds,
} from '../../apps/desktop/src/display';
import type { DisplayInfo } from '../../apps/desktop/src/display';
import { BRIDGE_VERSION } from '../../apps/game/src/platform/desktop';
import { NativeStorage } from '../../apps/desktop/src/storage';
...
describe('the relay and the desktop app agree on the desktop origin', () => {
  ...
  it('spells the same origin in both files', () => {
    const relayConfig = readFileSync(
      path.resolve(__dirname, '..', '..', 'apps', 'relay', 'src', 'config.ts'), 'utf8',
    );
    const m = /DESKTOP_ORIGIN\s*=\s*'([^']+)'/.exec(relayConfig);
    expect(m, 'apps/relay/src/config.ts must declare DESKTOP_ORIGIN as a literal').not.toBeNull();
...
describe('the packaged desktop renderer carries the production relay', () => {
  ...
  it('makes both package and preview builds use the configured renderer builder', () => {
    for (const file of ['dist.mjs', 'preview.mjs']) {
      const source = readFileSync(path.join(REPO, 'apps/desktop', file), 'utf8');
...
describe('the title-menu relay probe', () => {
  it('allows a cold WebGPU renderer to process the welcome before timing out', () => {
    const link = readFileSync(path.join(process.cwd(), 'apps/game/src/shell/net-link.ts'), 'utf8');
...
describe('desktop window branding', () => {
  ...
  it('gives the live BrowserWindow the VOLTMARCH mark in dev and packaged builds', () => {
    // electron-builder brands the release executable, but dev runs through the
    // generic electron.exe. Without a BrowserWindow icon Windows shows the
    // Electron logo in the title bar, taskbar and Alt+Tab even though releases
    // happen to look correct in Explorer.
    expect(main).toContain("path.join(DIST, 'brand', 'mark-512.png')");
    expect(main).toContain("'packages', 'assets', 'brand', 'mark-512.png'");
    expect(main).toMatch(/new BrowserWindow\(\{[\s\S]*?icon:\s*WINDOW_ICON/);
    expect(readFileSync(
      path.join(REPO, 'packages', 'assets', 'brand', 'mark-512.png'),
    ).byteLength).toBeGreaterThan(0);
  });
});

describe('desktop boot diagnostics', () => {
  it('forwards only errors and the bounded boot timing stream in development', () => {
    const main = readFileSync(path.join(REPO, 'apps', 'desktop', 'src', 'main.ts'), 'utf8');
    expect(main).toContain("details.message.startsWith('[boot]')");
    expect(main).toContain('[vm:renderer]');
  });
});
...
describe('the preload bridge and the game agree', () => {
  ...
  it('exposes every method the game\'s DesktopBridge declares', () => {
    ...
    const game = readFileSync(path.join(REPO, 'apps/game/src', 'platform', 'desktop.ts'), 'utf8');
...
describe('desktop shell import boundary', () => {
  ...
  it('NEVER imports game code — dist/ is served as opaque bytes', () => {
    ...
    for (const f of files) {
      ...
      for (const spec of imports) {
        expect(spec.includes('apps/game/src/'), `${f} imports game code: ${spec}`).toBe(false);
  ...
  it('is not reached INTO either — the game never imports the shell', () => {
     ...
     */
    const gameFile = path.join(REPO, 'apps/game/src', 'platform', 'desktop.ts');
    const src = readFileSync(gameFile, 'utf8');
    expect(/from\s+'[^']*desktop\/src/.test(src), 'apps/game/src/ imports the desktop shell').toBe(false);
    expect(/from\s+'electron'/.test(src), 'apps/game/src/ imports electron').toBe(false);
    // It must import nothing at all, in fact — see the file's own header.
    expect([...src.matchAll(/^\s*import\s/gm)].length, 'apps/game/src/platform/desktop.ts must import nothing').toBe(0);
  });
