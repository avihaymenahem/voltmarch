...
const SAVEGAME_CSS = readFileSync(join(ROOT, 'src/shell/savegame.css'), 'utf8');
const OBJECTIVES_CSS = readFileSync(join(ROOT, 'src/ui/objectives.css'), 'utf8');
const SHELL_TS = readFileSync(join(ROOT, 'src/shell/Shell.ts'), 'utf8');
...
function inMatchClassTokens(): Set<string> {
  ...
  const files = [
    ...
    'src/ui/Overlay.ts', 'src/ui/icons.ts', 'src/ui/Objectives.ts',
    'src/ui/ObjectiveBanner.ts', 'src/ui/PerfHud.ts',
  ];
