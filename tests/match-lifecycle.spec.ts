...
describe('every declared subscriber is still subscribed', () => {
  const SUBSCRIBERS = [
    ...
    'src/progression/MissionTracker.ts',
    'src/ui/Hud.ts',
  ];
  ...
  it('exactly one module emits it, and it is the lifecycle owner', () => {
    const emitters = [
      'src/game/outcome.system.ts', 'src/shell/Shell.ts', 'src/ui/Hud.ts',
      'src/audio/audio.system.ts', 'src/progression/MissionTracker.ts',
      'src/progression/progression.system.ts',
    ].filter((f) => source(f).includes("emit('match:started'"));
    expect(emitters).toEqual(['src/game/outcome.system.ts']);
  });
  ...
  it('the announcer arms its opening line from that handler and nowhere else', () => {
    ...
    const audio = source('src/audio/audio.system.ts');
