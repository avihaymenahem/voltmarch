...
const VFX_SYSTEM = readFileSync(join(ROOT, 'src/vfx/vfx.system.ts'), 'utf8');
...
describe('the call sites this unblocks', () => {
  it('are still there and still going through the port', () => {
    ...
    const files: ReadonlyArray<readonly [string, RegExp]> = [
      ...
      ['src/sim/Crates.ts', /vfx\.shake\(/],
      ['src/sim/RepairSell.ts', /vfx\.shake\(/],
      ['src/sim/Production.ts', /vfx\.play\(FxKind\.SellPuff/],
      ['src/sim/Abilities.ts', /vfx\.shake\(/],
    ];
