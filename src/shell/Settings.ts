  ...
  if (want('gameplay')) {
    ...
    configureRender({
      camera: {
        ...
        edgePanPixels: p.edgeScroll ? 8 : 0,
...
export class SettingsScreen implements Screen {
  ...
  private renderGameplay(body: HTMLElement): void {
    ...
    cam.appendChild(row(
      'Edge Scrolling',
      toggle(p.edgeScroll, (v) => set({ edgeScroll: v })),
      'Pan when the cursor reaches the screen edge.',
    ));
    cam.appendChild(row('Edge Scroll Speed', slider({
      min: 10, max: 120, step: 2, value: p.edgeScrollSpeed,
      format: (v) => `${Math.round(v)} m/s`,
      onChange: (v) => set({ edgeScrollSpeed: v }),
    })));
    cam.appendChild(row('Keyboard Pan Speed', slider({
    ...
    cam.appendChild(row(
      ...
      'How strongly the wheel pulls the point under the cursor to the centre.',
    ));
