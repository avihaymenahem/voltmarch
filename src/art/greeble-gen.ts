/**
 * ============================================================================
 * VOLTMARCH — src/art/greeble-gen.ts
 * ============================================================================
...
/* ==========================================================================
 * THE ATLAS, WITHOUT A RENDERER
 *
 * Everything above is pure typed-array maths over `Surface`, and this is the
 * function that runs the whole of it. It returns BUFFERS, not textures — the
 * `THREE.DataTexture` wrapping lives in `./Greeble.ts`, which is the only half
 * of the old file that needs a renderer.
 *
 * WHY THE SPLIT EXISTS: so this module can be imported by
 * `core/workers/textureWorker.ts`. Vite emits the worker and its import graph
 * as a separate chunk, and that chunk must not carry THREE — the same reason
 * `surfaces.ts` was split out of `assets.ts`, and the reason
 * `tests/texture-workers.spec.ts` walks the graph and fails on a stray import.
 *
 * Generating four 512² atlases costs ~960 ms of unbroken main-thread time at
 * boot, and the loading curtain cannot animate through it. Nothing here touches
 * the DOM, a canvas, `Math.random` or a wall clock, so it is safe to run on
 * another thread and produces byte-identical output wherever it runs — which
 * matters, because a lockstep match must not depend on where a texture was
 * baked.
 * ========================================================================== */

/** Everything `generateGreebleAtlas` produces. Structured-clone safe. */
export interface GreebleAtlasData {
  readonly key: string;
  readonly size: number;
  readonly spec: GreebleSpec;
  /** RGBA byte packings, ready to hand to a DataTexture. */
  readonly albedo: Uint8Array;
  readonly normal: Uint8Array;
  readonly orm: Uint8Array;
  readonly emissive: Uint8Array;
  /** Float fields the R1 gate re-measures per model. Not uploaded. */
  readonly surface: Surface;
  readonly structure: Float32Array;
  readonly metrics: GreebleMetrics;
}

/** Coverage figures measured during generation. Plain numbers. */
export interface GreebleMetrics {
...
export function greebleSpecHash(spec: GreebleSpec): string {
  return [
...
/**
 * Run every painter and every packer for one spec.
 *
 * `elapsedMs` is injected rather than read from a clock. `Date.now()` is banned
 * inside the sim for determinism reasons, and while this is not sim code, the
 * habit is worth keeping for a second reason here: the caller knows whether the
 * work happened on this thread or on a worker, and only the caller can time it
 * honestly.
 */
export function generateGreebleAtlas(spec: GreebleSpec, elapsedMs = 0): GreebleAtlasData {
  ...
  for (let y = 0; y < er.h; y++) {
    for (let x = 0; x < er.w; x++) {
      emissiveArea += s.emissive[((er.y + y) | 0) * size + ((er.x + x) | 0)];
    }
  }
...
export function greebleTransfers(d: GreebleAtlasData): ArrayBuffer[] {
  const out: ArrayBuffer[] = [];
  const push = (a: { buffer: ArrayBufferLike }): void => {
    const b = a.buffer;
    if (b instanceof ArrayBuffer && !out.includes(b)) out.push(b);
  };
  push(d.albedo); push(d.normal); push(d.orm); push(d.emissive);
  push(d.structure);
  const s = d.surface;
  push(s.albedo); push(s.height); push(s.roughness); push(s.metalness);
  push(s.ao); push(s.alpha); push(s.teamMask); push(s.emissive);
  return out;
}
