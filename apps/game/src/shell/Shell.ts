...
export class Shell {
  ...
  private async openMenu(firstBoot: boolean, keepBackdrop = false): Promise<void> {
    ...
    /*
     * FIRST PAINT IS A PRODUCT SURFACE, NOT A BATTLEFIELD BUILD.
     *
  ...
  private scheduleEnginePreload(): void {
    ...
    this.enginePreloadTimer = window.setTimeout(() => {
      ...
      void Promise.all([
        ...
        // A cache warm failure changes no state. The real launch will retry the
        // same imports and surface its ordinary loading error if it still fails.
        console.warn('[shell] engine module prefetch failed', err);
      });
    }, 1_000);
  ...
  private async bootGame(seed: number, backdrop: boolean): Promise<void> {
    ...
    // The lobby/replay/campaign has already resolved every occupied seat. Art
    // packs not represented here cannot appear in this world and must not load
    // dozens of GLBs merely because their system module exists.
    const artFactions = new Set<Faction>();
    const playerArt = factionByKey(this.setup.playerFaction)?.id;
    if (playerArt !== undefined) artFactions.add(playerArt);
    // The title backdrop seats exactly one opponent. Loading every faction
    // retained by a previous four-way lobby cannot change that two-army scene.
    const artOpponents = backdrop
      ? effectiveOpponents(this.setup).slice(0, 1)
      : effectiveOpponents(this.setup);
    for (const opponent of artOpponents) {
      const faction = factionByKey(opponent.faction)?.id;
      if (faction !== undefined) artFactions.add(faction);
    }
     ...
     */
    /*
     * THE ENGINE IS NOT PART OF THE TITLE SHELL'S CRITICAL BUNDLE.
     *
     * `Bootstrap` reaches `Systems.ts`, whose eager discovery glob imports all
     * 51 match systems and every faction-art module. A static import here made
     * "show a button" parse the complete game even after the backdrop itself
     * stopped blocking the menu. The dynamic boundary lets the shell paint and
    ...
    // On first load this reveals the canvas over the key art. On every later
    // boot the class is already absent, so this is a no-op.
    document.documentElement.classList.remove('vm-menu-preparing');
