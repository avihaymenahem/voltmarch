...
import { installPersistentPipelineCache } from './pipeline-retention';
...
export async function prepareRenderer(
  ...
  // The renderer/device intentionally survive match teardown. Three's default
  // cache policy does not: disposing the last material deletes its pipeline and
  // shader programs. Retain only those bounded caches so later matches reuse
  // compiled GPU work while all textures, meshes and bind groups still release.
  if (!installPersistentPipelineCache(renderer)) {
    console.warn(
      '[render] WebGPU pipeline retention unavailable; Three.js internals changed and ' +
      'later matches will recompile shaders',
    );
  }
