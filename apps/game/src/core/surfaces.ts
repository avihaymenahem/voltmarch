/**
 * ============================================================================
 * VOLTMARCH — src/core/surfaces.ts
 * ============================================================================
 * THE PROCEDURAL GENERATION CORE. PURE. NO THREE. NO DOM.
 *
 * This file was cut out of `assets.ts`, which had carried this note since it
 * was written:
 *
 *   "WORKER SAFETY: the generators below touch no DOM and no THREE. They
 *    operate on plain typed arrays, so they can be moved into a Web Worker
 *    wholesale (anything >= 256^2 should be) without modification."
 *
 * That was true, and it was only a comment — so nothing enforced it and nothing
 * used it. It is a MODULE BOUNDARY now: this file imports `./math` and nothing
 * else, so the texture worker in `./workers/` can import it without dragging
 * ~700 kB of Three.js into the worker chunk, and the day somebody reaches for
 * `THREE.Color` inside a generator the import graph says no.
 *
 * WHAT LIVES HERE   the Surface buffer, channel derivation, the drawing and
 *                   rasterization primitives, the noise budget, every
 *                   generator, the byte packers, and the request / cache-key
 *                   vocabulary those need.
 *
 * WHAT LIVES NEXT   `assets.ts` keeps everything that touches THREE: the
 * DOOR              `TextureFactory` (DataTexture construction, the GPU cache,
 *                   LRU eviction), canvas interop, and the material builders.
 *                   It re-exports this entire module, so every existing
 *                   `from './core/assets'` import still resolves exactly as
 *                   before. This split moved code, not API.
 *
 * THE PIPELINE
 * ------------
 ...
 *   3. Channels are packed into GPU-ready bytes per the packing standard below.
 *   4. `assets.ts` wraps those bytes in a `THREE.DataTexture` and caches it.
 *
 ...
 * Two runs produce byte-identical pixels — and because `generateTextureBytes`
 * at the bottom of this file is the ONE entry point both the main thread and
 * the worker call, two THREADS produce byte-identical pixels too.
 * `tests/texture-workers.spec.ts` asserts that rather than trusting it.
 * ============================================================================
 */

import {
  clamp01, clamp, lerp, smoothstep, fbm2, ridged2, simplex2, worley2,
  hexToRgb, hash2f, hash2i, Rng, TAU, mixInt, hexToInt, luminance,
} from './math';
...
export const DEFAULT_PARAMS: Required<AnyGenParams> = {
  size: 512, seed: 1,
...
export function b8(v: number): number {
...
export function stripUndefined(req: TextureRequest): Partial<AnyGenParams> {
...
/** Fill every generator parameter a request left out. */
export function resolveParams(req: TextureRequest): Required<AnyGenParams> {
  return { ...DEFAULT_PARAMS, ...stripUndefined(req) };
}

/**
 * Cache key for the raw float Surface — the CHANNEL is deliberately absent, so
 * albedo, normal and orm for one material all hit the same generated surface.
 */
export function surfaceKey(kind: TextureKind, p: Required<AnyGenParams>): string {
  return [
...
/**
 * Which normal packer a request gets. Clean kinds get the structural packer by
 * default; a caller can force either. Shared so the worker and the main thread
 * cannot drift on it — this is a pixel-affecting decision.
 */
export function usesStructuralNormal(req: TextureRequest): boolean {
  return req.structuralNormal ?? CLEAN_KINDS.has(req.kind);
}

/** Run a generator with no caching whatsoever. */
export function generateSurface(kind: TextureKind, p: Required<AnyGenParams>): Surface {
  const gen = GENERATORS[kind] ?? GENERATORS.checker;
  return gen(p);
}

/* ==========================================================================
 * THE ONE GENERATION ENTRY POINT
 *
 * Both threads call this. `TextureFactory.get()` calls it on the main thread
 * when no worker is available; `workers/textureWorker.ts` calls it inside the
 * worker. There is no second implementation to keep in step, which is what
 * makes "the worker produces the same bytes" a structural property rather than
 * a coincidence that a future edit could quietly break.
 * ========================================================================== */

/** GPU-ready bytes for one channel of one request. */
export interface TexturePixels {
  /** RGBA, length size*size*4. */
  data: Uint8Array;
  size: number;
...
/**
 * Pack one channel out of an already-generated surface.
 *
 * @param surface   the generated float surface
 * @param channel   which packing to produce
 * @param structural  use the noise-suppressing normal packer (see section 3D)
 * @param relief    height relief strength, for the normal channel only
 */
export function packChannel(
  surface: Surface, channel: Channel, structural: boolean, relief: number,
): TexturePixels {
  switch (channel) {
    case 'albedo': return { data: packAlbedo(surface), size: surface.size, srgb: true };
    case 'normal': return {
      data: structural ? packNormalStructural(surface, relief) : packNormal(surface, relief),
      size: surface.size,
      srgb: false,
    };
    case 'orm': return { data: packOrm(surface), size: surface.size, srgb: false };
    default: return { data: packMask(surface), size: surface.size, srgb: true };
  }
}

/**
 * Generate and pack one request, end to end.
 *
 * `surface` is an optimisation, not a variation: pass an already-generated
 * surface for the SAME request and the generator is skipped, which is how one
 * material's albedo + normal + orm share a single generator run. Passing a
 * surface from a different request would silently produce the wrong pixels, so
 * only callers that own the surface cache should use it.
 */
export function generateTextureBytes(req: TextureRequest, surface?: Surface): TexturePixels {
  const p = resolveParams(req);
  const s = surface ?? generateSurface(req.kind, p);
  return packChannel(s, req.channel ?? 'albedo', usesStructuralNormal(req), p.relief);
}

/**
 * The bytes a texture holds between "a worker was asked for it" and "the
 * worker answered": a flat, correctly-shaped, correctly-sized fill.
 *
 * In practice nothing ever renders these — boot waits for every dispatched job
 * before the loading curtain lifts — but "nothing ever renders it" is a
 * property of the boot sequence, not of this function, so the fill is the
 * honest neutral for each packing rather than transparent black:
 *
 *   ALBEDO / MASK   the request's own base colour, opaque, no team tint.
 *   NORMAL          flat facing (0.5, 0.5), cavity fully open, curvature flat.
 *   ORM             open AO, the request's roughness and metalness, no emissive.
 *
 * A frame that somehow slipped through would show untextured paint in the
 * right colour, which is the least alarming thing it could show.
 */
export function placeholderBytes(req: TextureRequest): TexturePixels {
  const p = resolveParams(req);
  ...
  if (channel === 'normal') {
    for (let i = 0; i < n; i++) {
      const o = i * 4;
      data[o] = 128; data[o + 1] = 128; data[o + 2] = 255; data[o + 3] = 128;
    }
    return { data, size: p.size, srgb: false };
  }

  if (channel === 'orm') {
    const rough = b8(p.roughness), metal = b8(p.metalness);
    for (let i = 0; i < n; i++) {
      const o = i * 4;
      data[o] = 255; data[o + 1] = rough; data[o + 2] = metal; data[o + 3] = 0;
    }
    return { data, size: p.size, srgb: false };
  }

  hexToRgb(baseColour(p), PLACEHOLDER_RGB);
  const r = b8(PLACEHOLDER_RGB[0]), g = b8(PLACEHOLDER_RGB[1]), b = b8(PLACEHOLDER_RGB[2]);
  for (let i = 0; i < n; i++) {
    const o = i * 4;
    data[o] = r; data[o + 1] = g; data[o + 2] = b; data[o + 3] = 255;
  }
  return { data, size: p.size, srgb: true };
}

/** Scratch for `placeholderBytes`. Never escapes this module. */
const PLACEHOLDER_RGB = new Float32Array(3);
