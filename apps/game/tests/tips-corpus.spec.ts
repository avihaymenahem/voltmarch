...
describe('§2 every row is a pair of predicates', () => {
  ...
  it('uses the budgets the brownout spec measured, not a second opinion', () => {
    const src = readFileSync(
      join(process.cwd(), 'apps/game/tests', 'tips-brownout.spec.ts'), 'utf8',
    );
...
describe('§5 a tip yields to an alert', () => {
  ...
  it('the HUD really publishes what the sim probes for', () => {
    const hud = readFileSync(join(process.cwd(), 'apps/game/src', 'ui', 'Hud.ts'), 'utf8');
    const chrome = readFileSync(join(process.cwd(), 'apps/game/src', 'ui', 'Chrome.ts'), 'utf8');
    const sim = readFileSync(join(process.cwd(), 'apps/game/src', 'sim', 'tips.system.ts'), 'utf8');
