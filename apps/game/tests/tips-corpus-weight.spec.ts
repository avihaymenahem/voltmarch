 ...
 */
const CORPUS = 'apps/game/src/sim/tip-rows.ts';
const DIRECTOR = 'apps/game/src/sim/tips.system.ts';
...
function walkSrc(): string[] {
  ...
  walk(join(ROOT, 'apps/game/src'), 'apps/game/src/');
  return out;
}
...
describe('the authored copy stays under a kilobyte', () => {
  ...
  it('is set below the declared leak it exists to prevent repeating', () => {
    ...
    const tutorialProse = stringLiteralBytes(read('apps/game/src/shell/tutorial-steps.ts'));
...
describe('the corpus module stays small enough to be worth shipping eagerly', () => {
  ...
  it('is set below the declared leak it exists to prevent repeating', () => {
    const leak = codeBytes(read('apps/game/src/shell/tutorial-steps.ts'));
...
describe('the failure names the way out', () => {
  ...
  it('points at the file that holds the argument', () => {
    expect(msg).toContain('apps/game/src/sim/tip-rows.ts');
  });
});
