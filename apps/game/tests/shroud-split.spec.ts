...
const FOG = read('apps/game/src/render/FogOfWar.ts');
...
describe('everything that draws above the ground tints itself', () => {
  ...
  it('entity props use the same shroud-aware material as scatter props', () => {
    ...
    const entities = code(read('apps/game/src/world/entity-props.system.ts'));
    const props = code(read('apps/game/src/world/PropLibrary.ts'));
  ...
  it('water carries the same formula, by its own route', () => {
    ...
    const water = read('apps/game/src/world/WaterMaterial.ts');
