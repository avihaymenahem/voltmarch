...
describe('the imported civilian building family', () => {
  it('ships the Oil Derrick and Hospital inside their explicit geometry and transfer budgets', () => {
    expect(manifest.assets.map((asset: any) => asset.key)).toEqual(['civ_derrick', 'civ_hospital']);
    expect(compression.rows).toHaveLength(2);
    expect(optimization.rows).toHaveLength(2);
  ...
  it('uses reviewed LOD0 silhouettes and bounded geometry-only shadow proxies', () => {
    for (const row of optimization.rows) {
      expect(row.outputs.some((output: any) => output.profile.startsWith('lod')), row.key).toBe(false);
  ...
  it('keeps both civilian replacements faction-neutral in runtime registration', () => {
