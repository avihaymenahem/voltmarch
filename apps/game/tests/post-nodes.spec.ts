...
describe('the assembled node chain', () => {
  ...
  it('gives AO a single-sample depth prepass when scene colour uses MSAA', () => {
    /*
     * WebGPU has no multisampled-depth resolve. Feeding the scene pass depth
     * into getNormalFromDepth used to compile it as
     * `texture_depth_multisampled_2d` and then emit the illegal
     * `textureDimensions(depth, 0)` overload. The colour pass remains MSAA;
     * only AO reads this separate depth-only pass.
     */
    const cfg = postConfigCopy();
    cfg.msaaSamples = 4;
    const g = graphFor(cfg);
    ...
    expect(compileFragmentNode(g.output).fragment).toContain('@fragment');
    g.dispose();
  });

  it('does not pay for an AO depth prepass without both AO and MSAA', () => {
    const noMsaa = graphFor(postConfigCopy());
    expect(noMsaa.aoDepthPass).toBeNull();
    noMsaa.dispose();

    const noAoCfg = postConfigCopy();
    noAoCfg.msaaSamples = 4;
    noAoCfg.ao.enabled = false;
    const noAo = graphFor(noAoCfg);
    expect(noAo.aoDepthPass).toBeNull();
    noAo.dispose();
  });
