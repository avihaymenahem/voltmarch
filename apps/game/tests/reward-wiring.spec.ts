...
const profileSource = code('apps/game/src/shell/Profile.ts');
const endScreenSource = code('apps/game/src/shell/EndScreen.ts');
const cosmeticMarksSource = code('apps/game/src/shell/CosmeticMarks.ts');
...
const lobbySource = code('apps/game/src/shell/SkirmishSetup.ts');
/** Map ids the lobby can actually offer. The catalogue, not the gate. */
const MAP_CATALOGUE: ReadonlySet<string> = new Set(
  Array.from(code('apps/game/src/shell/settings-store.ts').matchAll(/id:\s*'([a-z0-9-]+)',\s*name:/g))
    .map((m) => m[1]!),
);
const powerService = code('apps/game/src/sim/CommanderPowers.ts');
...
function claimFor(r: Reward): Claim | null {
  switch (r.kind) {
    case 'unlock': {
      ...
      if (isMapUnlockId(id)) {
        return {
          consumer: 'apps/game/src/shell/SkirmishSetup.ts#mapAvailable -> progression-link#isMapUnlocked',
      ...
      if (SUPERWEAPON_UNLOCK_IDS.includes(id)) {
        return {
          consumer: 'apps/game/src/data/Defs.ts#UNLOCK_TAGS -> progression/UnlockGate#allows',
    ...
    case 'map':
      return {
        consumer: 'apps/game/src/shell/SkirmishSetup.ts#mapAvailable',
    ...
    case 'credits':
      return {
        gap: 'NOTHING PAYS THESE. Thirteen match objectives retain credit metadata and no module '
          ...
          + 'simulation.',
        stillMissing: () => {
          for (const rel of ['apps/game/src/ui/Objectives.ts', 'apps/game/src/ui/ObjectiveBanner.ts', 'apps/game/src/shell/EndScreen.ts']) {
...
const productionSource = code('apps/game/src/sim/Production.ts');
