...
describe('determinism gate', () => {
  ...
  it('apps/game/src/sim/** contains no wall clock and no Math.random', () => {
    const files = walkTs(join(process.cwd(), 'apps/game/src', 'sim'), []);
...
describe('markup gate', () => {
  ...
  it('apps/game/src/** never turns a string into markup or into code', () => {
    const files = walkTs(join(process.cwd(), 'apps/game/src'), []);
