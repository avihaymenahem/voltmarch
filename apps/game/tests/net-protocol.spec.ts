...
describe('the relay can only reach the shared wire packages', () => {
  /**
   ...
   */
  it('the protocol imports nothing but game-types', () => {
    const src = read('packages/protocol/src/index.ts');
    const imports = [...src.matchAll(/^\s*import\s[^;]*?from\s+'([^']+)'/gm)].map((m) => m[1]);
    expect(imports.length).toBeGreaterThan(0);
    for (const spec of imports) {
      expect(spec, `protocol must not import ${spec}`).toBe('@voltmarch/game-types');
    }
  });

  it('game-types imports nothing at all, so the closure stays pure', () => {
    const src = read('packages/game-types/src/index.ts');
    const imports = [...src.matchAll(/^\s*import\s[^;]*?from\s+'([^']+)'/gm)].map((m) => m[1]);
    expect(imports, 'core/types.ts is the floor of the import graph').toEqual([]);
  });

  it('TurnRelay.ts stays inside the protocol package', () => {
    ...
    const src = read('packages/protocol/src/TurnRelay.ts');
    const imports = [...src.matchAll(/^\s*import\s[^;]*?from\s+'([^']+)'/gm)].map((m) => m[1]);
    for (const spec of imports) {
      expect(spec).toBe('./index');
    }
  });
});
...
describe('applyCommand covers every command the game can issue', () => {
   ...
   */
  const KINDS = ((): number[] => {
    const types = read('packages/game-types/src/index.ts');
