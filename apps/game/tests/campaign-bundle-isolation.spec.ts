...
describe.runIf(haveDist)('the built entry chunk carries no campaign', () => {
  ...
  it('is the real boot entry and points at the lazy product shell', () => {
    /*
     ...
     * The cold-start split intentionally reduced this file from ~2.9 MB to
     * ~5.7 kB and moved `policy.ts` behind the Shell/Bootstrap boundaries.
     * `Boot Failure` is owned by main.ts, and the fingerprinted Shell import is
     * the hand-off this entry must retain. Together they prove we read the live
     * entry without pinning it to the payload size this test exists to reduce.
     */
    const src = read(entry!);
    expect(src.length).toBeGreaterThan(4_000);
    expect(src.includes('Boot Failure'), 'main.ts boot failure seam is missing').toBe(true);
    expect(src).toMatch(/\.\/Shell-[A-Za-z0-9_-]+\.js/);
  });
