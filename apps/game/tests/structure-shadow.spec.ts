...
const SCENE_CODE = stripComments(read('apps/game/src/render/scene.ts'));
const BUILDINGS_CODE = stripComments(read('apps/game/src/art/buildings.system.ts'));
...
describe('the colour and depth programs share one copy of the maths', () => {
  const SRC = read('apps/game/src/art/BuildingFactory.ts');
