 ...
 * relative imports, as `file -> specifier` strings. The monorepo's pure
 * `@voltmarch/game-types` package is expected; renderer dependencies are not.
 */
...
describe('the worker import graph', () => {
  it('reaches only the pure shared game-types package — no THREE', () => {
    ...
    expect(bare).toEqual([
      `${join(SRC, 'core', 'types.ts')} -> @voltmarch/game-types`,
    ]);
  });
