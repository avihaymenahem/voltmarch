...
describe('page layout gate', () => {
  const SHELL_DIR = join(process.cwd(), 'apps/game/src', 'shell');
...
describe('a button built disabled', () => {
   ...
   * can pin exactly.
   */
  const shellSrc = readFileSync(join(process.cwd(), 'apps/game/src', 'shell', 'Shell.ts'), 'utf8');
  ...
  it('is what the Multiplayer entry actually uses', () => {
    const menu = readFileSync(join(process.cwd(), 'apps/game/src', 'shell', 'MainMenu.ts'), 'utf8');
