...
if [[ -e $release ]]; then
  ...
  if [[ $active == "$release" ]]; then
    ...
    [[ -f $release/dist/index.cjs && -f $release/dist/protocol.cjs && -f $release/smoke.mjs ]] \
      || die 'active release is incomplete'
    origin=$(sed -n 's/^VM_ORIGINS=//p' "$ENV_FILE" | cut -d, -f1)
    protocol=$(node -p "require('$release/dist/protocol.cjs').PROTOCOL_VERSION")
...
[[ -f $staging/dist/index.cjs && -f $staging/dist/protocol.cjs && -f $staging/smoke.mjs ]] \
  || die 'compiled relay is incomplete'
