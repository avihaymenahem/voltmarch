...
| Wave | Key | Asset | Route | Current state | Family plan |
| --- | --- | --- | --- | --- | --- |
| P1 | `tree` | Temperate Broadleaf | PROP | integrated | Geometry `01a04ec2-a4bb-77e8-b54f-400b74c82c33`, remesh `01a04ee5-dad8-7580-bf04-c71684f6957e`, PBR `01a04ee7-71ee-7606-b7c6-8d0f2572306d`: 3,363-triangle single-sided LOD0 with 1K/1K/512 maps, 802-triangle LOD1/caster and 384-triangle texture-free crossed LOD2/emergency. The complete 0.99 MiB family loads behind `?foliage=imported` through the stable Scatter identity and preserves procedural failure fallback. Direct low simplifications were visually rejected; camera-band LOD, authored PBR wind and WebGPU performance acceptance remain before deleting the builder. |
| P2 | `treeAutumn` | Autumn Broadleaf | PROP | integrated | 54/48/44-triangle card-and-trunk family, 40-triangle caster and shared ImageGen-derived `extended-foliage-v1` alpha PBR atlas. |
| P2 | `conifer` | Conifer | PROP | integrated | 82/64/52-triangle tiered branch family, 40-triangle caster and the shared extended-foliage atlas; LOD0 density was increased after cardinal review exposed an overly sparse first pass. |
| P2 | `palm` | Palm | PROP | integrated | 170/168/166-triangle segmented trunk/frond-card family with a 36-triangle caster and shared extended-foliage atlas. |
| P2 | `grassTuft`, `grassTuftGreen` | Grass Tufts | PROP | integrated | Two stable colour identities share 8/6/4-triangle crossed deliveries, 24-triangle closed casters and one ImageGen grass source in the extended-foliage atlas. |
| P1 | `bush` | Temperate Bush | PROP | integrated | Deterministic 28/16/6-triangle crossed-card LOD family with a 48-triangle closed caster. It shares the ImageGen-derived `temperate-shrub-v1` alpha-tested 1K/512/512 PBR material with `hedge`, including biome `COLOR_0` tint and no per-instance material. Stable Scatter identity and procedural failure fallback are preserved; Meshy spend was zero. |
...
| P2 | `haystack`, `containerStack`, `barrel` | Yard Props | PROP | integrated | Offline-baked authored silhouettes and 12-triangle casters share the ImageGen-derived metal/wood/hay/stone `prop-surface-v1` 1K/512/512 PBR atlas. The tent-like hay shelter was upgraded from a visibly faceted 12-sided mass to a 32-segment, smooth-normal 264-triangle silhouette. Its attempted reduced LOD exposed a large wall chord at gameplay distance, so the approved tiny topology is retained across colour LOD bands rather than reintroducing the defect for a negligible triangle saving. |
...
| P2 | `statue`, `statueRider`, `waterTower` | Civic Landmarks | PROP | integrated | Reviewed 616/836/1,778-triangle LOD0s with reduced far deliveries, 12-triangle casters and shared prop-surface PBR. |
