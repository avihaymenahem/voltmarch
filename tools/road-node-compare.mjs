/**
 * ============================================================================
 * VOLTMARCH — tools/road-node-compare.mjs
 * ============================================================================
 * THE CAPTURE HALF of the Stage D2 visual proof — the three ROAD marking
 * shaders, which Stage D's recount found assigned to no stage at all.
 *
 *   node tools/road-node-compare.mjs [--port 5365] [--headed] [--no-dither]
 *
 * Bundles `tools/road-node-compare/page.ts` with esbuild, serves it off a
 ...
 *
 * WHY A BROWSER, WHEN `tests/road-node-material.spec.ts` ALREADY BUILDS BOTH
 * BACKENDS. Because `WGSLNodeBuilder.build()` GENERATES a module and nothing in
 * Node compiles one, and three stages of this migration each found a different
 * way for a broken port to pass every offline gate — one of them shipping two
 * varyings as (0,0) with all 28 tests green. Offline generation is necessary and
 * NOT sufficient. Roads are also the surface it would cost the most to get
 * wrong: `docs/RENDER_FINDINGS.md` measures this generator at 3.80% edge density
 * against terrain's 0.96-1.59%, and every scrap of it is in these shaders.
 *
...
const PAGE_DIR = path.join(HERE, 'road-node-compare');
...
const START_PORT = Number(opt('--port', '5365'));
...
try {
  ...
  await page.evaluate(() => window.__RNC.ready);

  const report = await page.evaluate(() => ({
    arms: window.__RNC.arms,
    error: window.__RNC.error,
    warnings: window.__RNC.warnings,
  }));
