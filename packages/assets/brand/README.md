...
2. `hero-1920.webp` — replacement key art supplied 2026-08-26. 1823x863. Derived
   by `tools/splash.mjs`. It is the loading curtain and image-first title backdrop.
   It contains no baked wordmark, so the accessible DOM logo stays canonical.

**Neither source image is in this directory.** They are `tools/brand-source/logo-source.png`
and `tools/brand-source/splash-source.webp`, beside the scripts that read them. The first
...
| `splash-1600.webp` | 1600x757 | Loading curtain and title backdrop |
| `splash-640.webp` | 640x303 | The same, `srcset` entry for small viewports |
...
is a photographic illustration, where PNG is simply the wrong codec. It is also the one asset here that
...
`tools/splash.mjs` if the key art is. Replacements should contain no baked lettering: the
curtain and menu own one DOM wordmark at every aspect ratio.
